...
import { requireWriteAccess } from "../../../src/lib/permissions";
...
async function requireAdapterWriteAccess(extra: { authInfo?: AuthInfo }) {
...
const handler = createMcpHandler(
	(server) => {
		...
		server.tool(
			...
			{},
			async (_args, extra) => {
				await requireAdapterWriteAccess(extra);
				...
				if (!response.ok)
					throw new Error(
						(typeof body?.error === "string" && body.error) ||
							`Transaction refresh failed (${response.status})`,
					);
		...
		server.tool(
			"get-accounts",
			"List all connected bank accounts and credit cards.",
			{},
			async (_args, extra) => {
				const client = createUserClient(extra.authInfo!.token);
		...
		server.tool(
			"get-user-profile",
			"Get user's Prospify profile (age, income, credit score).",
			{},
			async (_args, extra) => {
				const client = createUserClient(extra.authInfo!.token);
		...
		server.tool(
			"get-linked-accounts",
			"Get confirmed linked accounts (primary/authorized user relationships).",
			{},
			async (_args, extra) => {
				const client = createUserClient(extra.authInfo!.token);
				...
				return text(
					(data ?? []).map((l) => ({
						id: l.id,
						primaryAccountId: l.primary_account_id,
						authorizedAccountId: l.authorized_account_id,
						primaryUserName: l.primary_user_name,
						authorizedUserName: l.authorized_user_name,
		...
		server.tool(
			...
			"List cards that have benefit tracking configured.",
			{},
			async (_args, extra) => {
				const client = createUserClient(extra.authInfo!.token);
				const userId = extra.authInfo!.extra?.userId as string;
		...
		server.tool(
			"get-benefit-summary",
			"YTD value captured for a card.",
			{ accountId: z.number().describe("Account ID") },
			async (args, extra) => {
				const client = createUserClient(extra.authInfo!.token);
				const { data: cd } = await client
					.from("credit_card_details")
					.select("card_id, credit_card_catalog(issuer, name)")
				...
				return text({
					card: { issuer: cat?.issuer, name: cat?.name },
					totalValueCaptured: Math.round(total * 100) / 100,
					viewYear: yr,
				});
			},
		);
		...
		server.tool(
			"get-benefit-details",
			"Detailed benefit configs and usage for a card at a given frequency.",
			{
				accountId: z.number(),
			...
			async (args, extra) => {
				...
				const { data: usages } = await client
					.from("benefit_usages")
					.select("benefit_config_id, amount_used")
					.in(
						"benefit_config_id",
						configs.map((c) => c.id),
					);
				...
				for (const u of usages ?? [])
					byConfig.set(
						u.benefit_config_id,
						(byConfig.get(u.benefit_config_id) ?? 0) + Number(u.amount_used),
					);
				return text(
					configs.map((c) => {
						...
						return {
							configId: c.id,
							benefitName: c.benefit_name,
							benefitKey: c.benefit_key,
							description: c.description,
							frequency: c.frequency,
							amount: amt,
							totalUsed: Math.round(used * 100) / 100,
							remaining: Math.max(0, Math.round((amt - used) * 100) / 100),
							usagePercent: amt > 0 ? Math.min(100, Math.round((used / amt) * 100)) : 0,
		...
		server.tool(
			"mark-benefit-used",
			"Manually mark a benefit as used.",
			{
		...
		server.tool(
			...
			"Auto-match benefits to transactions.",
			{ accountId: z.number() },
			async (args, extra) => {
				await requireAdapterWriteAccess(extra);
				const client = createUserClient(extra.authInfo!.token);
				...
				const { data: txs } = await client
					...
					.eq("account_id", args.accountId)
					.lt("amount", 0)
					.gte("date", ago.toISOString().slice(0, 10))
					.eq("is_deleted", false)
					.limit(5000);
				...
				for (const cfg of configs) {
					...
					for (const tx of txs ?? []) {
						...
						if (!patterns.some((p) => (tx.name as string).toLowerCase().includes(p.toLowerCase())))
							continue;
						const ps = new Date(now.getFullYear(), now.getMonth(), 1);
						const pe = new Date(now.getFullYear(), now.getMonth() + 1, 0);
						const { error } = await client
						...
						if (!error) {
							count++;
							used.add(tx.plaid_transaction_id);
						}
					}
				}
		...
		server.tool(
			...
			"Search credit transactions for benefit linking.",
			{
				accountId: z.number(),
		...
		server.tool(
			...
			async (_args, extra) => {
				...
				for (const tx of txs) {
					const k = `${tx.name.toLowerCase()}|${tx.account_id}`;
					const g: MerchantGroup = groups.get(k) ?? {
						name: tx.name,
						amounts: [] as number[],
						dates: [] as string[],
				...
				for (const [, g] of groups) {
					...
					const cv =
						Math.sqrt(g.amounts.reduce((s, a) => s + (a - avg) ** 2, 0) / g.amounts.length) / avg;
					if (cv > 0.2) continue;
					...
					if (med >= 27 && med <= 35) cadence = "monthly";
					else if (med >= 85 && med <= 100) cadence = "quarterly";
					else if (med >= 350 && med <= 380) cadence = "annual";
					else if (med >= 6 && med <= 8) cadence = "weekly";
					if (!cadence) continue;
					subs.push({
						merchantName: g.name,
						cardName: g.cardName,
						...
						lastSeen: g.dates[0],
						isActive:
							(Date.now() - new Date(sorted[sorted.length - 1]).getTime()) / 86400000 <= med * 1.5,
						category: g.category,
						confidence: Math.round((1 - cv) * 100),
					});
				}
				subs.sort((a, b) =>
					a.isActive !== b.isActive ? (a.isActive ? -1 : 1) : b.averageAmount - a.averageAmount,
				);
		...
		server.tool(
			...
			"Dismiss a false-positive subscription.",
			{ merchantKey: z.string().max(500), accountId: z.number() },
			async (args, extra) => {
				await requireAdapterWriteAccess(extra);
				const client = createUserClient(extra.authInfo!.token);
				const userId = extra.authInfo!.extra?.userId as string;
				const { error } = await client
					...
					.insert({ user_id: userId, merchant_key: args.merchantKey, account_id: args.accountId });
		...
		server.tool(
			"get-credit-matches",
			"Pending credit-to-charge match suggestions.",
			...
			async (args, extra) => {
				...
				return text({
					matches: data.map((m) => {
						const cr = txMap.get(m.credit_plaid_transaction_id);
						const ch = txMap.get(m.charge_plaid_transaction_id);
						return {
							suggestionId: m.id,
							...
							credit: cr
								? {
										name: cr.name,
										amount: Math.abs(Number(cr.amount)),
										date: cr.date,
										cardName: cr.card_name,
									}
								: null,
							charge: ch
								? {
		...
		server.tool(
			...
			"Confirm a credit-to-charge match.",
			...
			async (args, extra) => {
				...
				const { data: cr } = await client
					.from("transactions")
					.select("amount")
					.eq("plaid_transaction_id", s.credit_plaid_transaction_id)
					.maybeSingle();
				await client
					.from("transaction_credits")
					.insert({
						user_id: userId,
						plaid_transaction_id: s.charge_plaid_transaction_id,
						credit_plaid_transaction_id: s.credit_plaid_transaction_id,
						credit_amount: Math.abs(Number(cr?.amount ?? 0)),
					});
				await client
					.from("credit_match_suggestions")
		...
		server.tool(
			...
			"Credit transactions available for linking.",
			{ accountId: z.number().optional(), search: z.string().max(500).optional() },
			async (args, extra) => {
				const client = createUserClient(extra.authInfo!.token);
				let q = client
					.from("transactions")
		...
		server.tool(
			...
			async (args, extra) => {
				...
				const { error } = await client
					...
					.insert({
						user_id: userId,
						plaid_transaction_id: ch.plaid_transaction_id,
						credit_plaid_transaction_id: cr.plaid_transaction_id,
		...
		server.tool(
			"edit-transaction",
			"Edit a transaction's display name, amount, or date.",
			{
			...
			async (args, extra) => {
				...
				const { error } = await client
					...
					.upsert(od, { onConflict: "plaid_transaction_id" });
		...
		server.tool(
			...
			async (args, extra) => {
				...
				const { data: tx } = await client
					.from("transactions")
					.select("plaid_transaction_id, name, amount, date, account_id")
					.eq("id", args.transactionId)
					.maybeSingle();
				if (!tx) throw new Error("Transaction not found");
				const { error } = await client
		...
		server.tool(
			...
			async (args, extra) => {
				...
				return text(
					`Category changed to ${args.newCategory}${args.applyToAll ? " (applied to all)" : ""}.`,
				);
			},
		);
		...
		server.tool(
			...
			async (_args, extra) => {
				await requireAdapterWriteAccess(extra);
				return text(await syncSplitwiseForUser(extra.authInfo!.token));
			},
		);
		...
		server.tool(
			...
			async (_args, extra) => {
				...
				for (const m of members ?? []) {
					const l = mByG.get(m.group_id) ?? [];
					const f = fMap.get(Number(m.friend_id));
					if (f) l.push(f);
					mByG.set(m.group_id, l);
				}
				return text(
					(groups ?? []).map((g) => ({
						id: Number(g.id),
